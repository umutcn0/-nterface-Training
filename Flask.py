...
class Form(QDialog):
    ...
    def get_data(self):
        ...
        def secim(soup):
            ...
            self.browser.clear()
            for i in soup.findAll("div", {"class": compile(r'row.*')}):
                try:
                    if not (i.contents[1].string is None or i.contents[3].string is None):
                        if i.contents[1].string is None or i.contents[3].string is None:
                            ...
                            if i.contents[1].string == d[0] and i.contents[3].string == d[1]:
                                saat = str(i.contents[1].string)
                                program = str(i.contents[3].string) + "\n"
        ...
        def no_connection():
            self.browser.clear()
            self.browser.append("İnternet bağlantınızı kontrol edin.")
